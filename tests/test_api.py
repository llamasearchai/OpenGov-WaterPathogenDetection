"""FastAPI-specific tests for OpenGov-WaterPathogenDetection."""
...
class TestFastAPIIntegration:
    ...
    def test_api_items_crud_operations(self, client):
        ...
        print("[CRUD DEBUG] CREATE status=", response.status_code, "body=", response.text)
        assert response.status_code == 200, f"CREATE failed: {response.status_code} body={response.text}"
        ...
        print("[CRUD DEBUG] GET status=", response.status_code, "body=", response.text)
        assert response.status_code == 200, f"GET failed: {response.status_code} body={response.text}"
        ...
        print("[CRUD DEBUG] UPDATE status=", response.status_code, "body=", response.text)
        assert response.status_code == 200, f"UPDATE failed: {response.status_code} body={response.text}"

        # Delete item
        response = client.delete(f"/api/items/{item_id}")
        print("[CRUD DEBUG] DELETE status=", response.status_code, "body=", response.text)
        assert response.status_code == 200, f"DELETE failed: {response.status_code} body={response.text}"

        # Verify deletion
        response = client.get(f"/api/items/{item_id}")
        print("[CRUD DEBUG] GET after DELETE status=", response.status_code, "body=", response.text)
        assert response.status_code == 404, f"Expected 404 after delete got {response.status_code} body={response.text}"
